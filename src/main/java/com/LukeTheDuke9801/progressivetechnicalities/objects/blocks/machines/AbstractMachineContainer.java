...
public abstract class AbstractMachineContainer extends RecipeBookContainer<IInventory> {
	   ...
	   protected AbstractMachineContainer(ContainerType<?> containerTypeIn, IRecipeType<? extends AbstractCookingRecipe> recipeTypeIn, int id, PlayerInventory playerInventoryIn) {
	      this(containerTypeIn, recipeTypeIn, id, playerInventoryIn, new Inventory(3), new IntArray(3));
	   }

	   protected AbstractMachineContainer(ContainerType<?> containerTypeIn, IRecipeType<? extends AbstractCookingRecipe> recipeTypeIn, int id, PlayerInventory playerInventoryIn, IInventory furnaceInventoryIn, IIntArray furnaceDataIn) {
	      ...
	      this.addSlot(new Slot(furnaceInventoryIn, 0, 52, 35));
	      this.addSlot(new FurnaceResultSlot(playerInventoryIn.player, furnaceInventoryIn, 2, 117, 35));

	      for(int i = 0; i < 3; ++i) {
	         for(int j = 0; j < 9; ++j) {
	            this.addSlot(new Slot(playerInventoryIn, j + i * 9 + 9, 9 + j * 18, 86 + i * 18));
	         }
	      }

	      for(int k = 0; k < 9; ++k) {
	         this.addSlot(new Slot(playerInventoryIn, k, 9 + k * 18, 145));
	      }
	   ...
	   public int getBurnLeftScaled() {
	      ...
	      return 1; // this.furnaceData.get(0) * 13 / i;
	   }

	   @OnlyIn(Dist.CLIENT)
	   public boolean isBurning() {
	      return false; //this.furnaceData.get(0) > 0;
	   }
	}